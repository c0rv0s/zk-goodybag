...
describe("claim zk editions", function () {
  let sempahore;
  let editions;
  let verifier;
  ...
  before(async () => {
    ...
    // deploy zk editions
    const ZKEditions = await hre.ethers.getContractFactory("ZKEditions");
    editions = await ZKEditions.deploy();
    await editions.deployed();

    // deploy verifier
    const Verifier = await hre.ethers.getContractFactory("ZKEditionsVerifier");
    verifier = await Verifier.deploy(semaphoreAddress);
    await verifier.deployed();
  });

  it("create group", async () => {
    const tx = await verifier.createGroup(groupIds[0], editions.address, 0, 0);
    await expect(tx).to.emit(verifier, "GroupCreated");

    expect(await verifier.groupIdCollections(groupIds[0])).to.equal(
      editions.address
    );
  });
